...
class LitSegment(pl.LightningModule):
    def __init__(self, config):
        ...
        self.conf_model_roof = self.config['model_roof']
        ...
        self.model_roof = smp.create_model(self.conf_model_roof["model_name"], self.conf_model_roof["encoder_name"], encoder_depth=self.conf_model_roof["encoder_depth"], encoder_weights=self.conf_model_roof["encoder_weights"], in_channels=self.conf_model_roof["channels"], classes=1)

        self.loss = get_loss(self.conf_model_roof['loss'])

        params_roof = smp.encoders.get_preprocessing_params(self.conf_model_roof["encoder_name"], pretrained=self.conf_model_roof['encoder_weights'])
        self.register_buffer("std_roof", torch.tensor(params_roof["std"]).view(1, 3, 1, 1))
        self.register_buffer("mean_roof", torch.tensor(params_roof["mean"]).view(1, 3, 1, 1))
    ...
    def forward_roof(self, x):
        if self.conf_model_roof["encoder_weights"] is not None:
            x = (x - self.mean_roof) / self.std_roof
        mask = self.model_roof(x)
        return mask
    ...
    def shared_step(self, batch, mode):
        ...
        loss = self.loss(y_hat_roof, mask_roof)
